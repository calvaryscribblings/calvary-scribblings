...
export default function AdminPage() {
  ...
  const emptyForm = {
    ...
    date: formatDate(new Date()), coverFilename: '', content: '',
  };
  ...
  async function loadStories() {
    ...
    } catch (e) { setMsg('Error loading stories: ' + e.message); }
    setLoading(false);
  }
  ...
  const StoryForm = () => (
    <div>
      <div style={s.topBar}>
        ...
        <button style={s.btnGhost} onClick={() => { setView('list'); setMsg(''); }}>← Back</button>
      </div>
      ...
      <div style={s.form}>
        ...
        <div style={s.row2}>
          ...
          <div style={s.fg}>
            ...
            <div style={s.hint}>Upload the image to /public/ in the repo, then enter the filename here.</div>
          </div>
        </div>
        ...
        <div style={s.fg}>
          ...
          <div style={s.hint}>
            ...
            First paragraph flush, all subsequent paragraphs: style="text-indent:1.5em; margin-bottom:0"<br />
            Separator: &lt;p style="text-indent:1.5em; margin-bottom:0"&gt;***&lt;/p&gt;
          </div>
        </div>

        <div style={s.formActions}>
          <button style={s.btnGhost} onClick={() => { setView('list'); setMsg(''); }}>Cancel</button>
          <button style={{ ...s.btn, opacity: saving ? 0.6 : 1 }} onClick={saveStory} disabled={saving}>
        ...
        : stories.length === 0
          ...
          : stories.map(story => (
            <div key={story.id} style={s.card}>
              <img src={story.cover} alt={story.title} style={s.coverThumb}
              ...
              <div style={s.cardInfo}>
                ...
                <div style={s.cardMeta}>
                  ...
                  <a href={story.url} target="_blank" rel="noreferrer" style={{ color: '#a78bfa', textDecoration: 'none' }}>View →</a>
                </div>
              </div>
