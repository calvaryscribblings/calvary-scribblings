...
export async function onRequestPost(context) {
  ...
  if (!slug || !['story', 'reader'].includes(mode)) {
    return json({ error: 'slug and mode ("story" or "reader") are required.' }, 400);
  }
  ...
  try {
    ...
    return json({ error: `Claude returned invalid JSON: ${e.message}`, raw }, 500);
  }
